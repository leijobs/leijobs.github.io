...
为了进行后续 local map 融合，在投影过程计算量测的置信度，用于评估投影的语义点云的观测质量，基于以上计算，位置$(x_b,y_b)$相对于车辆中心的距离$\sqrt{x_b^2 + y^2_b}$可以表示为对像素$(u,v)$的函数$f(u,v)$；梯度$||\nabla f||$表示物理空间像素的尺度，较大的$||\nabla f||$表示像素到物理空间较大的不确定性，由于$||\nabla f||$与相机模型相关，因此通过归一化函数定义每个像素的量测置信度如下：

$$
bel([u,v])=\frac1{1+\exp(b(a-1/||\nabla f||))}
$$
...
$$
p^k(b^k)=p^k(b^k|l_{m_1}^{i_1},...,l_{m_{n-1}}^{i_{n-1}})+\log\frac{p(b^k|z_{l_{m_n}^{i_n}})}{1-p(b^k|z_{l_{m_n}^{i_n}})}
$$

其中，$p(b^k|z_{l_{m_n}^{i_n}})$表示线段的置信度
