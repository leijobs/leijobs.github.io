...
在 image-view 的数据增强中常用的手段为 flipping，cropping 和 rotation 等，对应引入的影响为一个 $3 \times 3$旋转矩阵$A$，可以表示为 $p'_{image}=Ap_{image}$ ；为了保证BEV视图下目标和特征的空间一致性，那么需要进行 $A^{-1}$ 变换从而还原数据增强的影响，那么：

$p'_{camera}=I^{-1}(A^{-1}p'_{image}*d)=p_{camera}$
