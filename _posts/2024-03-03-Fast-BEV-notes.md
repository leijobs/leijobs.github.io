...
![](static/H0StbW3FLoXUgTx07SdcUDcwnRc.png)
...
![](static/S4OXbf6t8oDAlYxGoePcJo98nLb.png)
...
![](static/NkLHb6rTBome7WxayxkctzA1n0e.png)
...
![](static/Z1vQbBcvpoqHKCxxoXLc6LK5nTf.png)

内容：重叠区域采用第一个视图的数据

#### Multi-Scale Image Encoder

![](static/Xc3Cbc3MWotWnhx5OKTc35dYnsb.png)
...
因此，通过 Fast-Ray Transformation 输出的 3 层网络的特征满足$F=\left\{R^{N \times \frac{X}{i} \times \frac{Y}{i} \times C} \mid X_{i} \in[4,8,16]\right\}$，对应通过 multi-scale projection 得到的 BEV 的特征满足$V=\left\{R^{X_{i} \times Y_{i} \times Z \times C} \mid X_{i}, Y_{i} \in[200,150,100]\right\}$
...
- S2C 操作：融合张量：$V=\left\{R^{X_{i} \times Y_{i} \times Z \times C} \right\} \Rightarrow V=\left\{R^{X_{i} \times Y_{i} \times ( ZC)} \right\}$
- MSCF 操作：融合多尺度特征，通过将特征上采样到同样维度来实现
- MFCF 操作：融合多帧特征：$Fuse(V_i \mid V_i \in R^{X_{i} \times Y_{i} \times ZCF_{scales}T_{frames}} ) \Rightarrow V_i \mid V_i  \in \left\{R^{X_{i} \times Y_{i} \times C_{MSCF \& MFCF} }  \right\} ，i \in 3 - level$
...
![](static/AzlhbeDn1oXq9bxLCsPce2asnod.png)

#### Temporal Fusion

参考 BEVDet4D 和 BEVFormer，Temporal 融合通过空间对齐和 concat 实现，每帧间隔为 0.5 秒

![](static/VXayb7EtXombl8xwm3HcZi93nzb.png)
...
![](static/VSwMbQBydoxpiFxfEkwceinOnx1.png)
