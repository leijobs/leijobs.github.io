...
其中，i 在所有关系矩阵中循环，而$w_{m}=\frac{\sum_{i}\left(1-A_{i}^{m}\right)}{\sum_{i} A_{i}^{m}}$
...
实际上，在计算中会分别优化语义 loss 函数$\mathcal{L}^{sem}_{\text {scal }}(\hat{y}, y)$，以及几何 loss 函数$\mathcal{L}^{geo}_{\text {scal }}(\hat{y}, y)$
