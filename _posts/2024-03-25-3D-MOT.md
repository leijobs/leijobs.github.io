...
> [https://github.com/hailanyi/3D-Multi-Object-Tracker](https://github.com/hailanyi/3D-Multi-Object-Tracker)
...
1. 检测得到的目标观测$X_t$
2. 通过上一帧预测$\hat{Z}_{t-1}$（包含跟踪$Z^*_{t-1}$和丢失$Z^{\prime}_{t-1}$）更新得到的当前的预测$\hat{Z}_t$
...
跟踪器要求包含完整状态的航迹预测$\hat{Z}_{t-1}$（包含跟踪$Z^*_{t-1}$和丢失$Z^{\prime}_{t-1}$），两者的关系为：
